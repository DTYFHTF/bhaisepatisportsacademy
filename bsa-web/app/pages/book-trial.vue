...
import { formatTime, addMinutesToTime } from '~/utils/formatters'

useSeoMeta({
  title: 'Book a Free Trial Session | Bhaisepati Sports Academy',
  description: 'Book a free trial session at BSA Bhaisepati. Try Badminton, Gym, or Sauna. Select your time, fill in your details, and confirm via WhatsApp.',
})
...
const trialType = ref<'badminton' | 'gym' | 'sauna' | null>(null)
...
const trialOptions = [
  ...
  { value: 'sauna' as const, label: 'Sauna' },
]
...
const canProceed = computed(() => {
  if (step.value === 'slot') return preferredDate.value && preferredTime.value && trialType.value
...
async function confirmBooking() {
  ...
  try {
    ...
    const response = await $fetch<{ trial_booking: { ref: string }; message: string }>(`${config.public.apiBase}/trial-bookings`, {
      method: 'POST',
      body: {
    ...
    trialRef.value = response.trial_booking.ref

    // Open WhatsApp with confirmation message including trial ID
    const message = `Hello, I confirm my trial booking.\n\nTrial ID: ${response.trial_booking.ref}\nActivity: ${trialType.value}\nDate: ${preferredDate.value}\nTime: ${timeRangeLabel.value}\nDuration: ${duration.value} min\nName: ${customerName.value}\nPhone: ${customerPhone.value}`
    ...
    <section class="border-b border-border">
      <div class="section-container py-8">
        ...
        <h1 class="font-display text-3xl sm:text-4xl uppercase tracking-tight text-ink">
          {{ step === 'slot' ? 'Book Your Free Trial Session' : step === 'details' ? 'Your Details' : 'Confirm Trial Booking' }}
        </h1>
      </div>
    </section>
      ...
      <div class="grid lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
          ...
          <div v-if="step === 'slot'" class="space-y-6">
            ...
            <!-- Trial Activity -->
            <div>
              <label class="text-xs font-medium uppercase tracking-wider text-ink-muted mb-2 block">Select Trial Activity</label>
              <div class="flex gap-2">
                <button
                  v-for="opt in trialOptions"
                  :key="opt.value"
                  class="rounded-lg border px-4 py-2 text-sm font-medium transition-colors"
                  :class="trialType === opt.value ? 'border-accent bg-accent text-canvas' : 'border-border text-ink-muted hover:border-accent/30'"
                  @click="trialType = opt.value"
                >
                  {{ opt.label }}
                </button>
              </div>
          ...
          <div v-else class="max-w-md space-y-5">
            ...
            <div class="rounded-2xl border border-accent/20 bg-accent/5 p-6 space-y-4">
              <div>
                <p class="text-xs font-medium uppercase tracking-wider text-accent mb-1">Trial Session</p>
                <p class="text-sm text-ink">{{ trialType ? trialOptions.find((o) => o.value === trialType)?.label : '' }}</p>
                <p class="text-sm text-ink">{{ preferredDate }}</p>
                <p class="text-sm text-ink-muted">{{ timeRangeLabel }}</p>
                <p class="text-sm text-ink-muted">{{ duration }} minutes</p>
              </div>
            ...
            <p class="text-sm text-ink-muted">
              Clicking confirm saves your trial booking and opens WhatsApp. Our team will confirm your slot.
            </p>
          </div>
        </div>
        ...
        <div class="lg:col-span-1">
          <div class="sticky top-24 rounded-2xl border border-border bg-surface p-5 space-y-4">
            ...
            <div v-if="!preferredDate || !preferredTime || !trialType" class="text-sm text-ink-muted py-4 text-center">
              Select an activity, date, and time to get started
            </div>

            <div v-else class="space-y-2">
              <div class="flex justify-between text-sm">
                <span class="text-ink-muted">Activity</span>
                <span class="text-ink capitalize">{{ trialType }}</span>
              </div>
              <div class="flex justify-between text-sm">
            ...
            >
              {{ step === 'confirm' ? 'Book Free Trial via WhatsApp' : 'Continue' }}
            </UiAppButton>
          </div>
        </div>
      </div>
